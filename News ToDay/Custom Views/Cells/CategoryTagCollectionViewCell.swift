...
class CategoryTagCollectionViewCell: UICollectionViewCell {
    ...
    let categoryButton: TwoStateButton = {
        ...
        button.setTitle("Category", for: .normal) // Set a default title
        button.setTitleColor(.white, for: .normal)
        button.layer.cornerRadius = 20 // Add rounded corners
        button.translatesAutoresizingMaskIntoConstraints = false // Important for constraints
        return button
    }()
...
class TwoStateButton: UIButton {
    ...
    private func configureButton() {
        ...
        self.addTarget(self, action: #selector(buttonDragOutside), for: .touchDragOutside)
    }
